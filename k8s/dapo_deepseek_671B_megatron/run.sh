...
# actor_ppo_max_token_len=$((1024 * 6)) #! 1017 会议决定更改
# infer_ppo_max_token_len=$((1024 * 12)) #! 1017 会议决定更改
# actor_ppo_max_token_len=$((1024 * 4)) #! 1017 会议决定更改
# infer_ppo_max_token_len=$((1024 * 4)) #! 1017 会议决定更改

enable_prefix_caching=False
####################

# Paths
# MODEL_PATH="/mnt/weight/deepseek3_bf16"
MODEL_PATH="/data/models/deepseek3_bf16"
DIST_CKPT_PATH="/mnt/share/z00804007/dsv3_base_bf16_mcore"

CKPTS_DIR=${JOB_LOG_DIR_CURR}/ckpts
TRAIN_FILE="/mnt/share/z00804007/data/gbs128-dapo-math-17k_dedup_r1_sys_prompt_mathdapo_filter_has_correct_answer.parquet"
TEST_FILE="/mnt/share/z00804007/data/gbs128-dapo-math-17k_dedup_r1_sys_prompt_mathdapo_filter_has_correct_answer.parquet"
...
COMMON_PP=${COMMON_PP:-1} #* GPU 是 16
...
# 运行时环境变量（会覆盖启动前的环境变量）
# RT_ENV_VARS='"VLLM_USE_V1": "1", "ASCEND_PROCESS_LOG_PATH": ' "${ASCEND_PROCESS_LOG_PATH}"

# if [ "${enforce_eager}" == "True" ]; then
#     RT_ENV_VARS="${RT_ENV_VARS}" ', "TASK_QUEUE_ENABLE": "1"'
# fi

# runtime_env_excludes='["/.git/", "/k8s", "/docker", "/doc", "/tmp", "/kernel_meta", "/plog"]'
# echo $RT_ENV_VARS
# echo $runtime_env_excludes
    ...
    # --runtime-env-json="{\"working_dir\": \"./\", \"excludes\": ${runtime_env_excludes}, \"env_vars\": {${RT_ENV_VARS}}}" \
ray job submit --runtime-env=/home/projects/verl/k8s/dapo_deepseek_671B_megatron/runtime_env.yaml \
    ...
    +actor_rollout_ref.model.override_config.model_config.num_nextn_predict_layers=0 \
    ...
    +actor_rollout_ref.actor.megatron.override_transformer_config.fused_permute_and_unpermute=True \
...
################## 将推理的结果保存下来，下一次训练可以直接读取这里的文件 #################
    # actor_rollout_ref.rollout.skip.enable=True \
    # actor_rollout_ref.rollout.skip.dump_dir="/mnt/share/s00894169/dump2" \
...
# ++actor_rollout_ref.ref.megatron.override_transformer_config.context_parallel_size=4 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.position_embedding_type='rope' \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.rope_scaling_type='yarn' \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.yarn_scaling_factor=40 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.reset_attention_mask=True \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.reset_position_ids=True \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.variable_seq_lengths=True \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.attention_mask_type='causal' \
...
#     +actor_rollout_ref.ref.megatron.override_transformer_config.rope_scaling_mscale=1.0 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.rope_scaling_mscale_all_dim=1.0 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.kv_lora_rank=512 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.v_head_dim=128 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.qk_rope_head_dim=64 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.qk_nope_head_dim=128 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.seq_length=2048 \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.sequence_parallel=True \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.fused_permute_and_unpermute=True \
#     +actor_rollout_ref.ref.megatron.override_transformer_config.moe_permutation_async_comm=True \
...
#     +actor_rollout_ref.rollout.skip_rollout.enable=True \
#     ++actor_rollout_ref.rollout.skip_dump_dir="/mnt/share/z00804007/dianxin/dump" \
#################################
# +actor_rollout_ref.actor.megatron.override_transformer_config.multi_latent_attention=True \